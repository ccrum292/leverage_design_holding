...
function SearchDiv(props) {
  ...
  const onSubmitSetHistoryNotProp = e => {
    e.preventDefault()
    if(search === ""){
      return console.log("No search text given")
    }
    let nextId;
  ...
  return (
    <div className="sm:flex sm:justify-center sm:h-screen fixed top-0 sm:w-full z-50 bg-gray-800 bg-opacity-75">
      <div className="fixed top-0 bg-red-700 z-20 h-full w-full  sm:max-w-4xl sm:self-center sm:rounded-lg">
        <div className="flex justify-between border-b border-gray-300">
          <div className="cursor-pointer p-2 self-center transition duration-300 ease-in-out transform hover:scale-125"
            onClick={() => { props.setSearchDivOpen(!props.searchDivOpen) }}>
          ...
          <div onClick={onSubmitSetHistoryNotProp} 
            className="cursor-pointer p-2 self-center transition duration-300 ease-in-out transform hover:scale-125">
          ...
          {searchHistory ? searchHistory.map(data => {
            return <div onClick={e => {
              ...
              <div className="cursor-pointer shadowClass z-10 flex justify-between w-full border-4 border-gray-300 rounded-full py-2 px-4 transition duration-500 ease-in-out transform hover:bg-gray-800">
                ...
                <FontAwesomeIcon onClick={e => { e.stopPropagation(); deleteFromSearchHistory(data); }} 
                  className="ml-2 cursor-pointer self-center transition duration-300 ease-in-out transform hover:scale-150" 
                  style={{ color: "#e2e8f0" }} size="1x" icon={faTimes} />
              </div>
            </div>
          }) : <div></div>}
